#ActiveState LOW RESOURCE WIN32 SPLASHSCREEN Receipt
import os
import sys
import win32gui
import win32api
...
class Splash:
    ...
    def OnInitDialog(self, hwnd, msg, wparam, lparam):
        ...
        centre_x, centre_y = win32gui.ClientToScreen(desktop, (int((dt_r - dt_l) / 2), int((dt_b - dt_t) / 2)))
        ...
        win32gui.SetWindowPos(self.hwnd, win32con.HWND_TOPMOST,
                              centre_x - int((self.bmWidth / 2)), centre_y - int((self.bmHeight / 2)),
    ...
    def EndDialogAfter(self, timeOut):
        ...
        thread = threading.Thread(target=self.Timer, args=(timeOut,))
        thread.daemon = True
        thread.start()
...
if __name__ == '__main__':
    s = Splash("splashscreen.bmp")
    thread = threading.Thread(target=s.DoModal)
    thread.daemon = True
    thread.start()
